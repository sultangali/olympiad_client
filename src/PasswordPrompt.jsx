...
const PasswordPrompt = ({ onPasswordCorrect }) => {
    const [input, setInput] = useState(localStorage.getItem('couratorPass') || '');
    const [error, setError] = useState(false);
    const correctPassword = '414717';
    useEffect(() => {
        // Проверяем сохраненный пароль сразу после монтирования компонента
    ...
    const handleSubmit = (event) => {
        ...
        if (input === correctPassword) {
            localStorage.setItem('couratorPass', input);
            onPasswordCorrect(true);
        } else {
            setError(true);
            setInput('');
